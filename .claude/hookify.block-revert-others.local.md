...
enabled: true
event: bash
pattern: (git\s+checkout\s+--\s+\.|git\s+restore\s+\.|git\s+reset\s+--hard|git\s+clean\s+-[a-z]*f)
action: block
---
...
2. 自分の変更だけを退避する（`git stash push <specific-files>`）
3. ワークスペースを作成して独立した環境で作業する（`git worktree add`）
