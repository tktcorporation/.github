...
OK: 「Discord API のレートリミット上、bulk fetch ではなく個別 fetch にする」
```
...
- [ ] 既存への参照を張れる箇所はないか
- [ ] PostToolUse hook（`mise run claude-postedit -- <file>`）を手動実行して違反ゼロを確認

### Phase 4: 完了確認

- [ ] `mise run claude-postedit -- <file>` が exit 0
- [ ] コード対象なら `npx sg scan <file>` が `No issues found`
...
| `rules/no-time-dependent-comments.yml` (ast-grep) | コードコメントの時間依存表現を検知 |
| `rules/no-historical-attribution-in-comments.yml` (ast-grep) | コードコメントの履歴・担当者参照を検知 |
| `.mise/tasks/claude-postedit` (PostToolUse hook) | `.md` 編集後に grep ベースで違反検知 |
